...
class Atom:
    ...
    # return: None
    def install(self):
        if self.preferred_source == "ubuntu-apt":
            self.mark_install_apt()
        elif self.preferred_source == "http-archive":
    ...
    def mark_install_apt(self):
        if "ppa" in self.object['package']['source']['ubuntu-apt']:
            import subprocess
    ...
    def get_apt_name(self):
        if "ubuntu-apt" in self.object['package']['source']:
            return self.object['package']['source']['ubuntu-apt']['name']
        else:
            return None

    def get_ppa(self):
        return self.object['package']['source']['ubuntu-apt']['ppa']